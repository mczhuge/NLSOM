...
#Please ensure that you will utilize all agents within the same community.
#Currently, our NLSOM has the following agents: [{tool_names}]. 
# If multiple agents exist within the same community, you MUST employ different agents to solve the same problem, thus incorporating diverse perspectives.
...
{ai_prefix}: 
Review: [Whether the NLSOM has solved the user-defined objective? Analyze the employed agents, NLSOM's organizational structure, and its input and output. ]
Mindstorm Result: [Provide a solution to the user-defined objective that utilizes all the agents' output. Don't miss the filenames if there are. Do not use "\n".]
